...
dados_totais = pd.read_csv(DIR+"nossas_casas.csv",sep=',')
criterios = pd.read_csv(DIR+"criterios.csv",sep=",",header=0,names=col_cri_names)
print(criterios)
...
for j in range(0,len(criterios)):
    ...
    treated_data = open(str(DIR+"nossas_casas_tratadas_"+str(j)+".csv"),"w")
